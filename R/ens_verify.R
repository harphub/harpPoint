...
#'   TRUE)`.
#' @param member_map_groupoings Logical. Whether to include `map_groupings` in
#'   the computation of scores for individual members when
#'   `verify_members = TRUE`. Defaults to `FALSE`.
...
ens_verify.harp_ens_point_df <- function(
  ...
  if (verify_members) {
    if (!is.null(map_groupings) && member_map_groupings) {
      det_map_groupings <- map_groupings
    } else {
      det_map_groupings <- NULL
    }
    det_summary_scores <- det_verify(
      .fcst,
      !!parameter,
      groupings          = groupings,
      map_groupings      = det_map_groupings,
      circle             = circle,
      summary            = summary,
      hexbin             = hexbin,
      num_bins           = num_bins,
  ...
  list_to_harp_verif(
    purrr::pmap(
      ...
      function(x, y, z) ens_verify(
        ...
        member_map_groupings = member_map_groupings,
        summary              = summary,
        circle               = circle,
        rel_probs            = rel_probs,
        num_ref_members      = num_ref_members,
