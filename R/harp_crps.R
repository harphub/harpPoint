...
harp_crps_alpha_beta <- function(eps, obs) {
  ...
  message("  Sorting members ", appendLF = FALSE)
  prev <- sort_members_df(eps)
  message(cli::col_green(cli::symbol[["tick"]]))

  message("  Computing alpha & beta ", appendLF = FALSE)
  index                     <- which(obs < prev[, 1])
  beta[index, 1]            <- prev[index, 1] - obs[index]
  index                     <- which(obs > prev[, nMember])
  alpha[index, nMember + 1] <- obs[index] - prev[index, nMember]
  ...
  out <- list(
    alpha      = alpha, #unname(split(alpha, 1:nrow(alpha))),
    beta       = beta, #unname(split(beta, 1:nrow(beta))),
    heaviside0 = heaviside0,
    heavisideN = heavisideN
  )
  message(cli::col_green(cli::symbol[["tick"]]))
  out
