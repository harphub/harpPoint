...
#'
#' @examples
lag_forecast <- function(.fcst, fcst_model, parent_cycles) {
  ...
  fcst_df <- fcst_df %>%
    ...
    dplyr::mutate(
      parent_cycle = purrr::map_chr(
        .data$fcst_cycle, ~ formatC(find_parent(as.numeric(.x), parent_cycles), width = 2, flag = "0")
      )
    )

  .fcst[[fcst_model]] <- purrr::map_dfr(split(fcst_df, fcst_df$parent_cycle), lag_cycle)
...
lag_cycle <- function(df) {
  parent_cycle   <- unique(df$parent_cycle)
  lags           <- df$fcst_cycle[df$fcst_cycle != parent_cycle]
  ...
  if (num_lags > 0) {
    ...
    for (i in 1:num_lags) {
      ...
      lag_hours <- as.numeric(parent_cycle) - as.numeric(lags[i])
      lag_hours[lag_hours < 0] <- lag_hours[lag_hours < 0] + 24

      lagged_df <- dplyr::inner_join(
        lagged_df,
        dplyr::filter(df, .data$fcst_cycle == lags[i]) %>%
          tidyr::unnest() %>%
          dplyr::select_if(~ !all(is.na(.))) %>%
          dplyr::mutate(
            leadtime = .data$leadtime - lag_hours,
            fcdate   = .data$fcdate   + lag_hours * 3600
          ) %>%
